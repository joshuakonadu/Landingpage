        ...
        <div class="mt-5 row justify-content-center">
            <div class="col-12 overflowX">
                ...
                <div v-if="messagesData.total > 10" class="col-12 mt-5">
                    <v-pagination v-model="page" :page-count="parseInt(messagesData.pages)" :classes="customClasses"> </v-pagination>
                </div>
            </div>
        </div>
        <stack-modal
...
export default {
    ...
    methods: {
        ...
        deleteMessage(id, index) {
            this.$store
                .dispatch("user/deleteNotification", { id, index })
                .then(() => {
                    this.reset();
                    this.$toast.open({
        ...
        openMessage(id, index, text) {
            this.$store.dispatch("user/updateMessageStatus", { id, index }).then(() => {
                this.text = text;
                this.show = true;
            });
        },
        ...
        reset() {
            (this.deleteShow = false), (this.deleteId = "");
            this.deleteIndex = "";
        },
    },
    computed: {
